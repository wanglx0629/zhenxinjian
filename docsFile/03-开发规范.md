...
- 多表一致性写操作必须加事务。
- **依赖风格（B-T19 裁定：实现类直依赖为主 + 接口化豁免清单）**：业务 Service 单一实现、无多态/替换点诉求时，调用方直接依赖 `service/impl` 实现类，不强抽接口（避免接口膨胀）；仅以下情形保持/新增接口——① 存在多实现或替换点（`SessionEvictor` Noop 适配、`GuestDataMigrator` 编排族多态 Ordered、`StorageService` 存储适配）；② 跨层契约/API 面需稳定（`UserService` 被多 controller+task 依赖且继承 MP `IService`、`WechatAuthService` 登录契约）。新增 Service 默认实现类直依赖，符合豁免情形才建接口。
- **controller / task 禁止直依赖 Mapper**：判定、查询一律下沉 Service 层（`task` 只做编排与外呼）。豁免两处基础设施类：`FoodLibraryInitializer`（ApplicationRunner 启动引导种子导入，非业务判定）、`GuestCleanupBatchExecutor`（B-T07 批级事务执行器，选批+删行即事务主体，下沉会割裂事务边界）。存量按新改动遵循，存量改造另立任务。
