// config/constants.js —— 全局常量。所有口径与 PRD / 高保真原型严格一致，修改前请同步更新 PRD。
...
/** 热量缺口档位（PRD §2.3，用户自选 X） */
const DEFICIT_OPTIONS = [200, 300, 400, 500]

/** 532 模式宏量配比（PRD §2.4） */
const MACRO_532 = { carb: 0.5, protein: 0.3, fat: 0.2 }
...
const CYCLE = {
  carbCoef: 2.5,          // 7 天总碳水 = 目标体重 × 2.5 × 周期天数
  fatCoefDefault: 0.8,    // 7 天总脂肪 = 目标体重 × 脂肪系数 × 周期天数（可切 1.0 更易执行）
  fatCoefOptions: [0.8, 1.0],
  proteinCoef: 1.5,       // 每日蛋白质 = 当前体重 × 1.5（周期内固定）
  // 高 / 中 / 低碳日的碳水 & 脂肪占比
  ratio: {
    high: { carbShare: 0.50, fatShare: 0.15, days: 2.0 },
    mid: { carbShare: 0.35, fatShare: 0.35, days: 2.2 },
    low: { carbShare: 0.15, fatShare: 0.50, days: 2.0 }
  },
...
module.exports = { ACTIVITY, DEFICIT_OPTIONS, MACRO_532, KCAL_PER_G, CYCLE, PERIOD_PHASES, RANGES, STAGE_532 }
