...
| ---- | ---- |
| 文档版本 | V1.2 |
| 编写日期 | 2026-09-04（V1.1 修订：2026-09-08 增补错误码分段 / 常量枚举 / 注释全量 / 分层强制规约；V1.2 修订：2026-09-15 增补 §1.2 JDK 版本与构建环境规约） |
| 适用工程 | `apps/zhenxinjian-backend`（Java 25 + Spring Boot 3.5.16） |
| 详细真源 | [docsFile/03-开发规范.md](../../../docsFile/03-开发规范.md)（本文件为约束摘要） |
...
### 1.2 JDK 版本与构建环境（mvn 执行前必读）

- **前提自检**：若当前系统默认 `JAVA_HOME` 已指向本项目的 JDK 25（`D:\App\java\25`），则跳过本段规约，无需额外处理。
- **背景**：本地同时开发与运行多个 Java 项目，系统默认 `JAVA_HOME` 指向 JDK 8（`D:\App\java\8`）；本项目使用 JDK 25（`D:\App\java\25`）。
- **规约**：执行 `mvn` 等构建 / 运行命令前，必须确认 JDK 版本与目标项目实际使用的版本一致——本项目一律显式指定 `JAVA_HOME=D:\App\java\25` 后再执行（否则报「无效目标发行版: 25」）；其他项目按各自实际使用的 JDK 版本处理，禁止想当然沿用系统默认值。
  - 示例（Windows cmd，环境变量随命令行生效）：`set JAVA_HOME=D:\App\java\25 && mvn test`
  - 会话型终端无法持久化环境变量时，可用临时批处理包装（先 `set JAVA_HOME` 再 `call mvn %*`）。
...
- 新增业务 CRUD：`gen-crud.js` 脚本恢复前按本规范 + [开发规范](../../../docsFile/03-开发规范.md) §3 手写完整样板（含 `data.sql` 与 `ExceptionConstant` 同步）。
...
> **文档版本**：V1.2　**最后更新**：2026-09-15　**维护**：臻心减项目组
