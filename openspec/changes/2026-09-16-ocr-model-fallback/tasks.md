...
- [ ] 1.2 本地库执行 change13 SQL 并验证种子/补账（mysql CLI，参照 change12 流程）
  - ⏸️ 2026-09-16 暂缓：需本地 MySQL，待环境就绪后补验
...
- [ ] 4.1 本地 E2E（错峰或临时配置一个稳定备用模型）：主模型 429 时自动切备用返回候选；后台改 `ocr.fallback-models` 即时生效；识别成功不扣减异常计数
  - ⏸️ 2026-09-16 暂缓：需本地 MySQL/Redis + 真实智谱 key，待环境就绪后补验
