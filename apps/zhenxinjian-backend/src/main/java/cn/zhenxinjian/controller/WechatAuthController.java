...
public class WechatAuthController {
    ...
    @Operation(summary = "微信一键授权登录",
            description = "wx.login() 的 code 换 openid 建号/找号并签发 token；"
                    + "multipart/form-data 表单，code/nickname/guestKey 走普通表单字段，avatar 为可选文件；"
                    + "可选 guestKey 触发游客数据迁移")
    @PostMapping(value = "/wechat/login", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    public Result<LoginResultVO> wechatLogin(@Valid @ModelAttribute WechatLoginDTO dto,
                                             @RequestPart(value = "avatar", required = false) MultipartFile avatar,
                                             HttpServletRequest request) {
        return Result.ok(wechatAuthService.wechatLogin(dto, avatar, JakartaServletUtil.getClientIP(request)));
    }
