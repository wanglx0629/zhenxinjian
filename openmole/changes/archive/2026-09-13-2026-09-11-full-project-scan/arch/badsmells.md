...
| 状态 | 已消除（B-T23 于 2026-09-13 完成，0634cc0 批 1 + 5cc81db 批 2 + 4dbcba3 批 3 + d965e95 批 4 四提交——批 1 骨架三件套：`constants/dicts.ts` 字典层（DictItem value/label/type + dictMap 码表 + dictLabel 兜底，十组枚举互锚）、`composables/usePageQuery.ts` 查询骨架（loading/数据/总数/查询重置/分页回调/删后回退/挂载首载七处托管）、`component/PagePager.vue` 通用分页条、`global.css` 三公共类；批 2 users 页迁移（七处样板收敛 + 五本地字典收敛 + PagePager + 四 scoped 改全局类 + PageResult 复用）；批 3 foods 页迁移 + API 类型收敛（七处样板收敛 + 三本地字典收敛 + categoryName() 删除改 dictLabel + adminFood/adminDiet 返回类型收敛 PageResult<T>）；批 4 diet-records 页迁移（七处样板收敛 + 两本地字典收敛 + displayUser 参数类型收窄 + 只读页无删后回退）；四批各自 vue-tsc + vite build 全绿，零行为变更） |
...
| 类别 | 层次（兼模块化） |
| 严重度 | 中 |
| 症状 | `docsFile/`（目录名拼写错误）+ `docs/knowledge/` + wiki（gitignore 排除独立仓库）三套文档并行且互引 |
| 证据 | `docsFile/01-项目介绍.md` 等 7 文件；`docs/knowledge/business-rule/invariants.md:21` 引用 `docsFile/projectFile/04-食物库数据字典.md`；`zhenxinjian.wiki/` 含同名第二份 |
| 根因 | 文档随不同阶段产物累积，无归属约定 |
| 影响 | 更新不知改哪份；新人困惑 |
| 修复建议 | 修正拼写并明确 `docs/` 为唯一真源；wiki 定位为只读镜像或删除 |
| 状态 | 已消除（B-T25 于 2026-09-13 完成，单提交——原拼写错误目录（dosc 前缀）git mv 更名 docsFile 保历史，全库 32 文件 48 处互引批量修正 grep 旧名清零；新建 .gitattributes 归属标记 docsFile/** attribution=project-narrative + [docsFile] 提交约定成文；docsFile 叙述性文档与 docs/ Harness 约束资产归属清晰裁定非副本不合并；zhenxinjian.wiki 独立仓库本地不存在裁定出本仓库范围；mvn compile + vue-tsc 双绿） |
...
| 状态 | 已消除（B-T26 于 2026-09-13 完成，单提交——`listMine` 加 `.last("LIMIT " + CommonConstant.CUSTOM_FOOD_MINE_LIMIT)` 有界查询；CommonConstant 新增 CUSTOM_FOOD_MINE_LIMIT = 200 单一真源（不复用 MAX_PAGE_SIZE=100，自定义食物列表与分页接口缩放特征不同）；消费方裁定小程序端 listMyCustomFoods → fetchMyCustom 一次性全量渲染非滚动分页，采 LIMIT 不改 API；新增 listMine_boundedByLimit 测试以 TableInfoHelper.initTableInfo 幂等初始化 + ArgumentCaptor 断言 SQL 片段含 LIMIT；mvn test 137 用例全绿，与 AGENTS.md「无上限 selectList」Never 条款对齐） |
...
| 状态 | 已消除（B-T30 于 2026-09-13 完成，单提交——扫描证据裁定有误：`git ls-files MRD-PRD`、`git log --all --full-history -- MRD-PRD`、`git ls-tree -r --name-only ee2a2fa`（扫描时点提交）三路核对零命中，MRD-PRD 目录从未被 git 追踪（磁盘存在但从未入库），无需 `git rm -r`；.gitignore 新增 `MRD-PRD/` 防复发规则（原型需求产物不入库，仅保留 PRD 文档在 docs/prd/）防未来误 `git add`；代码引用仅 `FoodLibraryInitializer.java:38` 注释来源标注，实际数据走 classpath 资源不依赖目录，文档散见引用属正常互引无需清理；后端 mvn test 全绿（EXIT_CODE=0）零影响；MRD-PRD 目录保留磁盘本地归档不移除，含 PRD docx/xlsx/pdf 与高保真原型 HTML，属需求产物非应用代码） |
...
| 状态 | 已消除（2026-09-13 B-T36，提交 84d21d8：新建 ADR 0005「微信 Token 内存态与单实例部署约束」——一期裁定单实例部署 `WxMaDefaultConfigImpl` 内存 token 为正式方案零协调成本；扩容前置门禁：任何多实例部署前必须切换 SDK 4.7.0 内置 `WxMaRedisConfigImpl`/`WxMaRedissonConfigImpl`（jar tf 实证 `cn.binarywang.wx.miniapp.config.impl` 包内置齐备），配置类替换业务代码零改动；与定时任务 ShedLock 化同属多实例扩展门禁族扩容评审一并核对；`WxMaConfiguration` 类注释互链 ADR 0005 双向可达；adr/README 索引登记文档版本 V1.0→V1.1） |
...
| v1.37 | 2026-09-13 | `84d21d8` | B-T36 完成，ARCH-演进-009 置"已消除"（新建 ADR 0005「微信 Token 内存态与单实例部署约束」：一期裁定单实例部署 WxMaDefaultConfigImpl 内存 token 为正式方案；扩容前置门禁——多实例前须切换 SDK 4.7.0 内置 WxMaRedisConfigImpl/WxMaRedissonConfigImpl，jar tf 实证 cn.binarywang.wx.miniapp.config.impl 包内置齐备，配置类替换业务零改动；与定时任务 ShedLock 化同属多实例扩展门禁族扩容评审一并核对；WxMaConfiguration 类注释互链 ADR 双向可达；adr/README 索引登记 V1.1）。**36 条全量清零，本轮全项目扫描收官** |
